...
const AskFromPdf = () => {
    ...
    const handleNewSession = async () => {
        try {
            const response = await fetch("/api/v1/chat/new", {
                ...
                body: JSON.stringify({ type: 2, sessionName: file.name }),
            });
            const data = await response.json();
            const newSessionId = data._id;
            setSessionId(newSessionId);
            setSessions((prevSessions) => [data, ...prevSessions]);
            navigate(`/askfrompdf/${newSessionId}`, { replace: true });
    ...
    const sendMessage = async (message, curr) => {
        ...
        const response = await fetch("/api/v1/chat/pdfchat", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
                        ...
                        {file && (
                            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                                ...
                                <button
                                    onClick={() => {
                                        setFile(null);
                                        document.getElementById("file-input").value = null; // Reset the input
                                    }}
